...
async function loadPayslipPayload(runId: string, employeeId: string) {
  ...
  // 1b) Compute SSP amounts for this run + employee using the new engine
  let sspForEmployee: any = null;

  if (companyId && periodStart && periodEnd) {
    try {
      const sspEmployeesRaw = await getSspAmountsForRun(
        companyId,
        periodStart,
        periodEnd,
      );

      const sspEmployees = Array.isArray(sspEmployeesRaw)
        ? sspEmployeesRaw
        : [];
  ...
  if (preError || !preRow) {
    return {
      ...
      body: {
        ...
        message:
          "No payroll record found for this employee in the selected run.",
  ...
  try {
    const { data: starterRow, error: starterError } = await supabase
      ...
      .select(
        "p45_provided, p45_present, starter_declaration, p45_tax_code",
      )
      .eq("employee_id", employeeId)
      .maybeSingle();
    ...
    if (row) {
      const p45Provided =
        row.p45_provided ?? row.p45_present ?? false;
      if (p45Provided) {
        ...
        if (decl === "A") {
          starterTaxCode = "1257L Cumulative";
        } else if (decl === "B") {
          starterTaxCode = "1257L wk1/mth1";
        } else if (decl === "C") {
          starterTaxCode = "BR wk1/mth1";
        }
      }
    }
  } catch (err) {
  ...
  const elementRows: any[] = Array.isArray(rawElementRows)
    ? rawElementRows
    : [];
  ...
  for (const row of elementRows) {
    ...
    const baseTaxable =
      typeof master?.["taxable_for_paye"] === "boolean"
        ? master["taxable_for_paye"]
        : true;
    const baseNi =
      typeof master?.["nic_earnings"] === "boolean"
        ? master["nic_earnings"]
        : true;
    const basePensionable =
      typeof master?.["pensionable_default"] === "boolean"
        ? master["pensionable_default"]
        : true;
    const baseAe =
      typeof master?.["ae_qualifying_default"] === "boolean"
        ? master["ae_qualifying_default"]
        : true;
    ...
    const taxableForPaye =
      typeof row["taxable_for_paye_override"] === "boolean"
        ...
        : baseTaxable;
    const nicEarnings =
      typeof row["nic_earnings_override"] === "boolean"
        ? row["nic_earnings_override"]
        : baseNi;
    const pensionable =
      typeof row["pensionable_override"] === "boolean"
        ? row["pensionable_override"]
        : basePensionable;
    const aeQualifying =
      typeof row["ae_qualifying_override"] === "boolean"
    ...
    const normalised: NormalisedPayElement = {
      ...
      aeQualifying,
      isSalarySacrificeType: Boolean(
        master?.["is_salary_sacrifice_type"] ?? false,
      ),
  ...
  // 6) Override SSP element amount AND description if the SSP engine has a result
  let sspAdjusted = false;
  let sspDetails: any = null;

  if (sspForEmployee && sspForEmployee.totalPayableDays > 0) {
    ...
    if (sspElem) {
      sspElem.amount = sspForEmployee.sspAmount;

      const payable = sspForEmployee.totalPayableDays;
      const qualifying = sspForEmployee.totalQualifyingDays;
      sspElem.name = "Statutory Sick Pay";
      sspElem.description = `Statutory Sick Pay for ${payable} day${
        payable === 1 ? "" : "s"
      } sickness (${qualifying} qualifying day${
        qualifying === 1 ? "" : "s"
      } in this period)`;
      ...
      sspDetails = {
        ...
        totalPayableDays: sspForEmployee.totalPayableDays,
        dailyRate: sspForEmployee.dailyRate,
        amount: sspForEmployee.sspAmount,
      };
      ...
      console.log("payslip route: SSP element adjusted", {
        ...
        code: sspElem.code,
        name: sspElem.name,
        description: sspElem.description,
        newAmount: sspElem.amount,
      });
    }
  }
  ...
  const finalTaxCode =
    rawTaxCode || starterTaxCode || "1257L wk1/mth1";
  ...
  const deductionsElements = elements.filter(
    (e) => e.side === "deduction",
  );
  ...
  const deductionElementsTotal = deductionsElements.reduce(
    (sum, e) => sum + (Number(e.amount) || 0),
    0,
  );
  ...
  const useElementTotals =
    (grossStored === 0 && earnings.length > 0) || sspAdjusted;
  ...
  const payAfterLeaving =
    typeof preRow["pay_after_leaving"] === "boolean"
      ? preRow["pay_after_leaving"]
      : false;
  ...
  const fullName =
    firstName && lastName
      ? `${firstName} ${lastName}`
      : firstName || lastName || "Unnamed employee";

  const niNumber =
    employeeRow["ni_number"] ??
    employeeRow["national_insurance_number"] ??
    null;
