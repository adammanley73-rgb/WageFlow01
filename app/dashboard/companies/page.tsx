...
function adminClient() {
  ...
  if (!url || !key) {
    throw new Error("companies page: missing Supabase env");
  }
...
async function getCompanies(): Promise<Company[]> {
  try {
    ...
    if (error) {
      console.error("companies page: error loading companies", error);
      return [];
    }

    return data ?? [];
  } catch (err) {
    console.error("companies page: admin client error", err);
    return [];
  }
}
