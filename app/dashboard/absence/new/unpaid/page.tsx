...
export default function UnpaidLeaveWizardPage() {
  ...
  function validate() {
    ...
    if (!String(form.totalDays || "").trim()) {
      next.totalDays = "Total days is required";
    } else {
      const n = Number(form.totalDays);
      if (!Number.isFinite(n) || n <= 0) next.totalDays = "Enter a valid number of days";
    }
  ...
  async function handleSubmit(e: any) {
    ...
    try {
      ...
      const res = await fetch("/api/absence/unpaid", {
        method: "POST",
      ...
      if (!res.ok || data?.ok === false) {
        const msg =
          data?.message ||
          data?.error ||
          "The server could not save this unpaid leave record.";
      ...
      <div className={CARD}>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <section className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              ...
              <div>
                ...
                />
                {errors.totalDays ? (
                  <p className="mt-1 text-xs text-red-600">{errors.totalDays}</p>
                ) : null}
              </div>
            </div>
          </section>
          ...
          <div className="mt-2 flex justify-end gap-3">
            ...
            >
              {submitting ? "Saving..." : "Save unpaid leave"}
            </button>
          </div>
        </form>
      </div>
