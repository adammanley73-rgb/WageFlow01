...
if (!form.weeksOfLeave.trim()) {
  ...
  if (!Number.isFinite(n) || n <= 0) nextErrors.weeksOfLeave = "Enter a valid number of weeks";
  if (Number.isFinite(n) && n > 2) nextErrors.weeksOfLeave = "Paternity leave is typically 1 or 2 weeks";
}
...
try {
  ...
  if (!res.ok || data?.ok === false) {
    const message = data?.message || data?.error || "The server could not save this record.";
...
return (
<PageTemplate title="Paternity leave" currentSection="absence" headerMode="wizard" backHref="/dashboard/absence/new" backLabel="Back" >
<div className="flex flex-col gap-4 flex-1 min-h-0">
<div className={INTRO_CARD}>
<div className="text-sm text-neutral-700">
Record paternity leave for an employee. This creates an absence record now and will later feed statutory pay scheduling when that layer is fully wired.
</div>
</div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-neutral-700 mb-1">Weeks of leave</label>
              <input
                type="number"
                min="1"
                max="2"
                ...
                className="w-full rounded-xl border border-neutral-300 px-3 py-2 text-sm text-right focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-blue-600"
                placeholder="1 or 2"
              />
              {errors.weeksOfLeave && (
                <p className="mt-1 text-xs text-red-600">{errors.weeksOfLeave}</p>
              )}
              <p className="mt-1 text-[11px] text-neutral-600">
                Most cases are 1 or 2 weeks. This field supports validation and later statutory scheduling.
              </p>
            </div>
          </div>
        </section>
        ...
        <div className="flex flex-col md:flex-row gap-3 md:items-center md:justify-between mt-2">
          <p className="text-[11px] text-neutral-600">
            This records the absence with the intended dates. Statutory pay elements (SPP) will be generated during the statutory pay wiring phase.
          </p>
