...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'pay_schedules table does not exist, skipping migration';
    RETURN;
  END IF;
  ...
  EXECUTE 'ALTER TABLE public.pay_schedules DROP CONSTRAINT IF EXISTS pay_schedules_frequency_check';
  EXECUTE 'ALTER TABLE public.pay_schedules ADD CONSTRAINT pay_schedules_frequency_check CHECK (frequency IN (''weekly'',''fortnightly'',''four_weekly'',''monthly''))';
END $$;

-- 1) Employees: make pay_schedule_id the truth (only if pay_schedules exists).
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables 
  ...
  -- Check if employees has pay_schedule_id column
  IF NOT EXISTS (
  ...
  BEGIN
    ...
    -- Only set frequency column if it exists
    IF EXISTS (
      SELECT 1 FROM information_schema.columns 
      WHERE table_schema = 'public' 
      AND table_name = 'employees' 
  ...
  BEFORE INSERT OR UPDATE OF pay_schedule_id, pay_frequency ON public.employees
  FOR EACH ROW
  ...
  ELSE
    UPDATE public.employees e
    SET pay_frequency = ps.frequency
    FROM public.pay_schedules ps
    WHERE e.pay_schedule_id = ps.id
      AND e.pay_frequency IS DISTINCT FROM ps.frequency;
  END IF;
END $$;

-- 2) Payroll runs: add an optional pay_schedule_id (only if pay_schedules exists).
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables 
  ...
  SET pay_schedule_id = (
    ...
    WHERE ps.company_id = pr.company_id
      ...
      AND ps.frequency = pr.pay_frequency
      AND (
...
-- 3) Helper: detect "run activity" (always create this in wf_util).
...
-- 4) Draft-only auto-sync function (only create if pay_schedules exists).
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables 
    ...
    RAISE NOTICE 'pay_schedules table does not exist, skipping attach_due_employees_to_run function';
    RETURN;
  END IF;
