do $$
begin
  if to_regclass('public.employee_bank') is not null then
  ...
  if to_regclass('public.employee_bank') is not null then
    for v_constraint in
      ...
      where n.nspname = 'public'
        ...
        and c.conname <> 'employee_bank_payment_method_check'
    loop
  ...
  if to_regclass('public.employee_bank_accounts') is not null then
    for v_constraint in
      ...
      where n.nspname = 'public'
        ...
        and c.contype = 'c'
        and pg_get_constraintdef(c.oid) ilike '%payment_method%'
        and c.conname <> 'employee_bank_accounts_payment_method_check'
    loop
