...
function Fix-HeaderBanner {
  param([string]$content)

  # Remove ANY import that includes HeaderBanner (default/named, any path)
...
function Fix-StyleObjectConstAssertion {
  param([string]$content)

  # Add "as const" to common style object declarations to avoid CSSProperties union errors
  # Pattern: const S = { ... };
  $rxConstS = @'
(?s)const\s+S\s*=\s*\{.*?\}\s*;
'@
  ...
  $content = [regex]::Replace(
    ...
    $rxConstS,
    {
      param($m)
      $txt = $m.Value
      if ($txt -match 'as\s+const\s*;') {
        return $txt  # already has const assertion
      }
      # Replace the last "};" with "} as const;"
      $txt -replace '\}\s*;\s*$', '} as const;'
    }
  )
...
# Style object candidates: any TSX under dashboard/settings or dashboard/employees that has "const S =" and "style={S."
$styleFiles = @(
  Select-String -Path "app/**/*.tsx" -Pattern "const\s+S\s*=\s*\{","style=\{S\." -SimpleMatch -ErrorAction SilentlyContinue |
    Select-Object -ExpandProperty Path -Unique
)
...
foreach ($file in $bannerFiles) {
  ...
  $new = Fix-HeaderBanner -content $txt
  if ($new -ne $txt) {
...
foreach ($file in $apiFiles) {
  $txt = Get-Content -Raw -LiteralPath $file
  $new = Fix-NextRequest -content $txt
  if ($new -ne $txt) {
    Set-FileContentUtf8 -Path $file -Content $new
...
foreach ($file in $styleFiles) {
  $txt = Get-Content -Raw -LiteralPath $file
  $new = Fix-StyleObjectConstAssertion -content $txt
  if ($new -ne $txt) {
    Set-FileContentUtf8 -Path $file -Content $new
