// C:\Projects\wageflow01\components\payroll\bulk-operations.tsx
"use client";
...
interface BulkOperationsProps {
  ...
  onBulkAction: (action: string, selectedRuns: string[], newStatus?: WorkflowStatus) => Promise<void>;
  onRefresh: () => void;
}

export function BulkOperations({ payrollRuns, onBulkAction, onRefresh }: BulkOperationsProps) {
  ...
  const executeBulkAction = async () => {
    ...
    try {
      ...
      alert(`✅ Successfully ${bulkAction.replace(/_/g, " ")} ${selectedRuns.length} payroll run(s)`);
    } finally {
      setProcessing(false);
    }
  };
      ...
      <div style={styles.controls}>
        <div style={styles.checkboxContainer}>
          <input
            type="checkbox"
            id="selectAll"
            checked={selectedRuns.length === payrollRuns.length && payrollRuns.length > 0}
        ...
        <select value={bulkAction} onChange={(e) => setBulkAction(e.currentTarget.value)} style={styles.select}>
          <option value="">Choose Action...</option>
          <option value="start_processing">🔄 Start Processing</option>
          <option value="approve_all">✅ Approve All</option>
          <option value="submit_rti">📤 Mark RTI Submitted</option>
        ...
        <button
          style={{
            ...styles.button,
            opacity: processing || selectedRuns.length === 0 || !bulkAction ? 0.6 : 1,
            cursor: processing || selectedRuns.length === 0 || !bulkAction ? "not-allowed" : "pointer",
          }}
        ...
        {selectedRuns.length > 0 && (
          <button style={styles.buttonSecondary} onClick={() => setSelectedRuns([])}>
            Clear Selection
          </button>
        )}
      </div>
      ...
      <div style={styles.list}>
        {payrollRuns.map((run) => {
          ...
          return (
            ...
            >
              <input type="checkbox" checked={isSelected} onChange={() => handleSelectRun(run.id)} />
